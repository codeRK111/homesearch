...
export const getAllSurvays = (data, cancelToken, setLoading) => {
	const token = localStorage.getItem('JWT');
	const stringified = queryString.stringify(data);
	return new Promise((resolve, reject) => {
		setLoading(true);
		axios
			.get(apiUrl(`/project/get-all-opinions?${stringified}`, 'v2'), {
				cancelToken: cancelToken.token,
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bearer ${token}`,
...
export const updateSurvay = (id, data, cancelToken, setLoading) => {
	const token = localStorage.getItem('JWT');
	return new Promise((resolve, reject) => {
		setLoading(true);
		axios
			.patch(apiUrl(`/project/update-opinion/${id}`, 'v2'), data, {
				cancelToken: cancelToken.token,
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bearer ${token}`,

...
const RentBasicInfo = ({ onSubmit, initialData }: IRentBasicInfo) => {
	...
	useEffect(() => {
		...
		const location = initialData.location ? initialData.location.name : '';
		const city = initialData.city ? initialData.city.name : '';
		setPdata({
			...pData,
			...initialData,
			...
			furnished,
			sale_type: type,
			title: getTitle(bedrooms, type, location, city),
			rent,
		});
	}, []);
	return (
